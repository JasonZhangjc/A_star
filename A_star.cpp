...
{
    int extraG=(abs(node->x - temp_start->x) + abs(node->y - temp_start->y)) 
                == 1 ? costH : costD;
...
{
    open_list.push_back(new Node(startNode.x, startNode.y));
	// put the source node in and open a new node
	
    while(!open_list.empty())
    {
        auto curNode = getLeastFNode();     // find the node with min F
        open_list.remove(curNode);        // delete current node from open_list
        close_list.push_back(curNode);    // put current node into close_list
        ...
        {
              /* for a node, if it's not in open_list, put it into open_list,
			   * and set current node as its parent, calculate F,G,H      */
            if(!isInList(open_list, target))
            {
			  ...
			  /* for a node, if it's in open_list, calculate G,
            ...
            Node *resNode=isInList(open_list, &endNode);
            if(resNode)
                return resNode; 
                // return resNode instead of endNode, because of the deep copy
        }
    }
...
bool Astar::isCanReach(const Node *node, const Node *target,
